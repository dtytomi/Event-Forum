...
export class ThreadComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    var self = this;
    self.dataService.getThreadRef().child(self.thread.key).on('child_changed', self.onCommentAdded);
  }

  ngOnDestroy() {
    console.log('destroying..');
    var self = this;
    self.dataService.getThreadRef().child(self.thread.key).off('child_changed', self.onCommentAdded);
  }  
  ...
  public onCommentAdded = (childSnapshot, prevChildKey) => {
    ...
    self.thread.commets = childSnapshot.val();
  }
